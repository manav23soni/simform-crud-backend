...
const { ERROR422, ERROR500, ERROR400, ERROR401 } = require("../../../constants/common");
...
middleware.checkDuplicateRouteName = async (req, res, next) => {
  try {
    ...
    const findRouteName = await dynamicRouteService.findRouteName(routeName);
    if (findRouteName) {
      return res.status(ERROR400.CODE).json({
...
middleware.checkValidation = (req, res, next) => {
  ...
  if (!utils.isEmpty(bodyParameters)) {
    ...
    for (let i = 0; i < routeParameters.length; i++) {
      ...
      let findIndex = requireValidations.findIndex((x) => x === routeParameters[i].field)
      if (findIndex === -1) {
